...
function isString(value) {
  ...
  const valueObj = value.valueOf();
  return typeof valueObj === 'string';
}
...
function isPalindrome(str) {
  return (
    str.toLowerCase().replaceAll(/\W/g, '').split('').reverse().join('') ===
...
function encodeToRot13(str) {
  const enter =
    'ABCDEFGHIJKLMNOPQRSTUVWXYZNOPQRSTUVWXYZABCDEFGHIJKLMabcdefghijklmnopqrstuvwxyznopqrstuvwxyzabcdefghijklm';
  let newStr = '';
  const step = 26;
  for (let i = 0; i < str.length; i += 1) {
